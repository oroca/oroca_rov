...
final int VIEW_SIZE_X = 600, VIEW_SIZE_Y = 800;
...
void draw_model()
{
  ...
  translate(VIEW_SIZE_X/2, 300, 0);
  
  // Rotate shapes around the X/Y/Z axis (values in radians, 0..Pi*2)
  rotateX(radians(imu_x));  //roll
  rotateZ(radians(imu_y));   //pitch
  rotateY(radians(imu_z));
...
int cube_x_width = 20/2;
int cube_y_width = 30/2;
int cube_height  = 5/2;
...
void buildBoxShape() 
{
  ...
  fill(#00ff00);
  vertex(-cube_x_width, -cube_height, cube_y_width);
  vertex(cube_x_width, -cube_height, cube_y_width);
  vertex(cube_x_width, cube_height, cube_y_width);
  vertex(-cube_x_width, cube_height, cube_y_width);
...
void drawCube() {  
  pushMatrix();
    ...
    rotateY( radians(imu_z));
    rotateX( radians(imu_x));
    rotateZ( radians(imu_y));
